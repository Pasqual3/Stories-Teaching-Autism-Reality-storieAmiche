...
const ActionButtons = ({ isSubmitting, onAddScene, onSaveDraft, onPublish, firstSceneEmpty = false }) => {
        ...
        <div className="mt-10 flex flex-wrap justify-between items-center gap-4 pt-8 border-t border-gray-100">
            ...
            <div className="flex flex-wrap gap-4">
                ...
                <button
                    type="button"
                    onClick={onPublish}
                    disabled={isSubmitting || firstSceneEmpty}
                    title={firstSceneEmpty ? "Aggiungi almeno un testo alla prima scena" : ""}
                    className={`flex items-center gap-2 px-8 py-3 rounded-full font-bold shadow-lg text-white transition-all ${
                        isSubmitting || firstSceneEmpty
                            ? 'bg-gray-400 cursor-not-allowed opacity-60'
                            : 'bg-gradient-to-r from-purple-500 to-indigo-600 hover:scale-105 hover:shadow-indigo-200/50'
                    }`}
                >
                    {isSubmitting ? (
